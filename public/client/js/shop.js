(function ($) {
    ...
    var rangeTwoPrice = function () {
        if ($("#price-value-range").length > 0) {
            ...
            skipSlider.noUiSlider.on("update", function (val, e) {
                skipValues[e].innerText = val[e] + " VNĐ"; // Thêm VNĐ vào giá trị
            });
        }
    };
    ...
    var filterProducts = function () {
        ...
        priceSlider.noUiSlider.on("update", function (values) {
            ...
            $("#price-min-value").text(filters.minPrice + " VNĐ"); // Hiển thị VNĐ
            $("#price-max-value").text(filters.maxPrice + " VNĐ"); // Hiển thị VNĐ
        ...
        function updateMetaFilter() {
            ...
            if (filters.minPrice > minPrice || filters.maxPrice < maxPrice) {
                appliedFilters.append(
                    `<span class="filter-tag remove-tag" data-filter="price"><span class="icon-close"></span>Price: ${filters.minPrice} VNĐ - ${filters.maxPrice} VNĐ</span>`
                );
            }
            if (filters.color) {
                appliedFilters.append(
        ...
        function applyFilters() {
            ...
            $(".wrapper-shop .card-product").each(function () {
                ...
                const priceText = product
                    ...
                    .replace(/\./g, ""); // Loại bỏ dấu chấm (nghìn phân cách)
            ...
            $("#product-count-grid").html(
                `<span class="count">${visibleProductCountGrid}</span> ${visibleProductCountGrid === 1 ? "Product" : "Products"
                } found`
            );

            $("#product-count-list").html(
                `<span class="count">${visibleProductCountList}</span> ${visibleProductCountList === 1 ? "Product" : "Products"
                } found`
            );
    ...
    var filterSort = function () {
        ...
        function applyFilter(sortValue, isListActive) {
            ...
            if (sortValue === "price-low-high") {
                products.sort((a, b) => {
                    const priceA = parseFloat($(a).find(".price-new").text().replace(/[^0-9]/g, ""));
                    const priceB = parseFloat($(b).find(".price-new").text().replace(/[^0-9]/g, ""));
                    return priceA - priceB; // So sánh theo thứ tự giá từ thấp đến cao
                });
            } else if (sortValue === "price-high-low") {
                products.sort((a, b) => {
                    const priceA = parseFloat($(a).find(".price-new").text().replace(/[^0-9]/g, ""));
                    const priceB = parseFloat($(b).find(".price-new").text().replace(/[^0-9]/g, ""));
                    return priceB - priceA; // So sánh theo thứ tự giá từ cao đến thấp
                });
